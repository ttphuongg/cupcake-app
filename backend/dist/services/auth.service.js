...
export const authService = {
    ...
    verifyRegister: async (email, _otp) => {
    ...
    forgotPassword: async (email) => {
        ...
        // Lưu vào DB
        await pool.execute('UPDATE Users SET reset_token = ?, reset_token_expires_at = ?, reset_token_type = ?, last_reset_request_at = NOW() WHERE id = ?', [token, expiresAt, 'forgot_password', user.id]);
    ...
    verifyResetToken: async (token) => {
        const [rows] = await pool.query('SELECT * FROM Users WHERE reset_token = ? AND reset_token_type = ?', [token, 'forgot_password']);
