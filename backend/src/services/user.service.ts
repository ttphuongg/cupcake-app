...
export const userService = {
    ...
    requestChangePasswordLink: async (userId: number) => {
        ...
        const resetLink = `http://192.168.2.1:3000/api/v1/user/change-password-redirect?token=${token}`;
    ...
    requestDeleteAccountLink: async (userId: number, password: string) => {
        ...
        const resetLink = `http://192.168.2.1:3000/api/v1/user/delete-account-redirect?token=${token}`;
